...
export default function SuccessView({ onRestart, userName }: SuccessViewProps) {
  return (
    <div id="success-view" className="flex flex-col h-full bg-[#0a0d16] text-[#eef2f6] px-5 py-6 justify-between select-none items-center text-center">
      {/* Top spacing */}
      <div />

      {/* Main Confetti / Star Medal Core layout */}
      <div className="flex flex-col items-center gap-4 py-6 max-w-xs relative">
        ...
        <div className="space-y-3 mt-4">
          <p className="text-[11px] uppercase tracking-widest font-mono text-cyan-400 font-semibold bg-cyan-950/40 px-3 py-1 rounded-full border border-cyan-800/30 inline-block">
            MỞ KHÓA HOÀN TẤT
          </p>
          <h2 className="text-xl font-display font-black text-white px-2 leading-snug">
      ...
      <div className="w-full space-y-2 max-w-xs">
        <button
          ...
          onClick={onRestart}
          className="w-full bg-slate-900 hover:bg-slate-850 text-slate-200 font-display font-semibold text-sm py-3.5 px-4 rounded-xl shadow border border-slate-800 transition-all duration-200 active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
        >
